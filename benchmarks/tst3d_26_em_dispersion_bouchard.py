...
def fx(x, y, z):
    if (Ly/2.2 < np.mod(y,Ly) <= Ly/2.2+dy and Lz/2.2 < np.mod(z,Lz) <= Lz/2.2+dz):
        return 1
    else:
        return 0

def fy(x, y, z):
    if (Lz/2.2 < np.mod(z,Lz) <= Lz/2.2+dz and Lx/2.2 < np.mod(x,Lx) <= Lx/2.2+dx):
        return 1
    else:
        return 0

def fz(x, y, z):
    if (Lx/2.2 < np.mod(x,Lx) <= Lx/2.2+dx and Ly/2.2 < np.mod(y,Ly) <= Ly/2.2+dy):
        return 1
    else:
        return 0
...
Main(
    ...
    #],
    EM_boundary_conditions = [['PML','PML'],['PML','PML'],['PML','PML']],
    number_of_pml_cells    = [[10,10],[10,10],[10,10]],
    pml_kappa = [lambda x : 1 + 79 * x**4],
    pml_sigma = [lambda x : 20 * x**2],
    solve_poisson = False,
    print_every = 10,
)
...
Antenna(
    field = "Jx",
    space_profile = fx,
    time_profile = tcosine(base=0., start=t0, duration=t0/2, freq=2,phi=pi/2)
)
